...
import { DEMO_BARCODES, foodFromBarcode } from "@/lib/barcodes";
...
export function ScanBoard() {
  ...
  const apply = (raw: string, add = false) => {
    const food = foodFromBarcode(raw);
    if (!food) {
      setMsg(t(locale, "notFoundBarcode"));
      return;
    }
    setMsg(`${foodName(food, locale)}${food.brand ? ` · ${food.brand}` : ""}`);
  ...
  return (
    <div className="space-y-6">
      <header className="animate-rise">
        ...
        <p className="mt-1 text-xs text-ink-soft">{t(locale, "barcodeHint")}</p>
      </header>
