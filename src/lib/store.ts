...
export type FarfurieState = State;
...
const storeCreator: StateCreator<State> = (set, get) => ({
      ...
      toggleHoliday: () => set((s: State) => ({ holidayMode: !s.holidayMode })),
      ...
      addEntry: (entry: Parameters<State["addEntry"]>[0]) => {
      ...
      addFoodToMeal: (foodId: string, meal: MealKey, grams?: number) => {
      ...
      addRecipeToMeal: (recipeId: string, meal: MealKey) => {
        const recipe =
      ...
      addCustomFood: (meal: MealKey, name: string, macros: Macros) => {
      ...
      toggleTheme: () =>
        set((s: State) => ({ theme: s.theme === "dark" ? "light" : "dark" })),
      setDayNote: (date: string, note: string) =>
        set((s: State) => ({ dayNotes: { ...s.dayNotes, [date]: note } })),
      logWeight: (kg: number) => {
      ...
      saveMealFromSelected: (meal: MealKey, name: string) => {
        const trimmed = name.trim();
        if (!trimmed) return;
      ...
      addSelectedDayToShopping: () => {
        ...
        set((s: State) => ({ shopping: [...s.shopping, ...items] }));
      },
      copyMealToDate: (meal: MealKey, targetDate: string) => {
      ...
      setTargetWeight: (kg: number) => {
        if (kg < 30 || kg > 250) return;
        set({ targetWeightKg: Math.round(kg * 10) / 10 });
      },
      saveNamedMeal: (name: string, items: SavedMeal["items"]) => {
        const trimmed = name.trim();
      ...
      setProgramMode: (mode: ProgramMode) => set({ programMode: mode }),
      setDietStyle: (style: DietStyle) =>
        set((s: State) => ({
          dietStyle: style,
          goals: macrosForStyle(s.goals.kcal, s.profile.weightKg, style),
        })),
      setWeeklyRatePct: (pct: number) => set({ weeklyRatePct: Math.min(1, Math.max(0.25, pct)) }),
      toggleTrainingDay: (day: DayKey) =>
        set((s: State) => ({
      ...
      quickAdd: (meal: MealKey, macros: Macros) => {
      ...
      copyDayToDate: (targetDate: string) => {
        if (targetDate > shiftISO(localISO(), 7)) return 0;
        const source = get().selectedDate;
      ...
      logMeasurement: (row: Parameters<State["logMeasurement"]>[0]) => {
      ...
      addUserRecipe: (recipe: Recipe) =>
        set((s: State) => ({ userRecipes: [recipe, ...s.userRecipes].slice(0, 40) })),
      toggleSleep: (date?: string) => {
      ...
      setEnergy: (level: 0 | 1 | 2 | 3, date?: string) => {
        const day = date ?? get().selectedDate;
        set((s: State) => {
          const current = s.habitsByDate[day] ?? { sleep: false, energy: 0 as const };
          return {
      ...
      habitsFor: (date: string) => get().habitsByDate[date] ?? { sleep: false, energy: 0 },
      exportSnapshot: () => {
        const s = get();
        return {
          v: 1,
      ...
      importSnapshot: (snapshot: FarfurieSnapshot) => {
        set({
      ...
      setProfile: (profile: ProfileInput) => set({ profile }),
      applyProfileGoals: () => {
        const s = get();
      ...
      setPlanSlot: (day: DayKey, meal: MealKey, recipeId: string | null) =>
        set((s: State) => {
      ...
      clearCheckedShopping: () =>
        set((s: State) => ({ shopping: s.shopping.filter((i) => !i.checked) })),
      clearShopping: () => set({ shopping: [] }),
      setFastingProtocol: (id: FastingProtocol["id"]) => set({ fastingProtocolId: id }),
      startFasting: () => set({ fastingStartedAt: new Date().toISOString() }),
      stopFasting: () => set({ fastingStartedAt: null }),
      logExercise: (exerciseId: string, minutes: number) => {
...
export function useEffectiveGoals() {
  return useFarfurieStore(useShallow((s: FarfurieState) => s.effectiveGoals()));
}

export function useTotals() {
  return useFarfurieStore(useShallow((s: FarfurieState) => s.totals()));
}

export function useRemaining() {
  return useFarfurieStore(useShallow((s: FarfurieState) => s.remaining()));
}
